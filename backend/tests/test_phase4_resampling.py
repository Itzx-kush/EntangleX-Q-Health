...
class ResamplingTests(unittest.TestCase):
    ...
    def test_none_preserves_training_rows(self):
        result=self.apply(self.X,self.y,"none",42,1);self.assertEqual(result.rows_after,10);self.assertEqual(result.distribution_after,{"0":8,"1":2})
    ...
    def test_smote_balances_with_valid_neighbors(self):
        result=self.apply(self.X,self.y,"smote",42,1);self.assertEqual(result.distribution_after,{"0":8,"1":8});self.assertEqual(result.rows_added,6)
