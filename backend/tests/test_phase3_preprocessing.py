...
class PhaseThreePreprocessingTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.service = PreprocessingService(
            self.datasets,
            PreprocessingRepository(f"sqlite:///{root / 'test.db'}"),
            root / "uploads",
            root / "artifacts",
        )
    ...
    def dataset(self) -> dict:
        rows = ["age,marker,group,target"]
        rows.extend(
            f"{30 + index},{1 + index % 5},{'A' if index % 2 else 'B'},{index % 2}"
            for index in range(40)
        )
        rows.append("69,5,A,1")
        content = ("\n".join(rows) + "\n").encode()
        summary = self.ingestion.ingest("study.csv", content)
        return self.ingestion.select_target(summary["id"], "target")
    ...
    def test_pipeline_is_fitted_on_training_only_and_persisted(self) -> None:
        ...
        artifact_path = self.service.artifact_dir / Path(result["artifact_path"]).name
...
"""
        ...
        with self.assertRaises(PreprocessingError) as context:
            self.service.run(PreprocessingConfig(dataset_id=summary["id"]))
