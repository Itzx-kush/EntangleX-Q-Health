...
class FeatureReductionTests(unittest.TestCase):
 ...
 def test_variance_filter_fits_training_only(self):
  result=self.reduce(self.X,self.test,self.y,self.names,"variance",3,0.0,"none",2,42);self.assertNotIn("constant",result.selected_feature_names);self.assertEqual(result.train.shape[1],3)
 ...
 def test_mutual_information_is_deterministic(self):
  first=self.reduce(self.X,self.test,self.y,self.names,"mutual_info",2,0,"none",2,42);second=self.reduce(self.X,self.test,self.y,self.names,"mutual_info",2,0,"none",2,42);self.assertEqual(first.selected_feature_names,second.selected_feature_names)
 def test_pca_reports_real_explained_variance(self):
  result=self.reduce(self.X,self.test,self.y,self.names,"variance",3,0,"pca",2,42);self.assertEqual(result.output_feature_names,["PC1","PC2"]);self.assertEqual(len(result.explained_variance_ratio),2);self.assertLessEqual(sum(result.explained_variance_ratio),1.0000001)
