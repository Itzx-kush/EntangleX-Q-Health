...
class PhaseThreeApiTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        datasets = DatasetRepository(f"sqlite:///{root / 'api.db'}")
        ingestion = DatasetService(datasets, root / "uploads", 2)
        preprocessing = PreprocessingService(
            datasets,
            PreprocessingRepository(f"sqlite:///{root / 'api.db'}"),
            root / "uploads",
            root / "artifacts",
        )
        app.dependency_overrides[get_dataset_service] = lambda: ingestion
        app.dependency_overrides[get_preprocessing_service] = lambda: preprocessing
    ...
    def test_preprocessing_run_api(self) -> None:
        rows = ["age,group,target"]
        rows.extend(f"{30 + index},{'A' if index % 2 else 'B'},{index % 2}" for index in range(30))
        content = ("\n".join(rows) + "\n").encode()
        uploaded = self.client.post(
            "/api/datasets/upload",
            files={"file": ("study.csv", content, "text/csv")},
        )
        ...
        response = self.client.post(
            ...
            json={"dataset_id": dataset_id, "random_seed": 42},
        )
        self.assertEqual(response.status_code, 201, response.text)
        body = response.json()
        self.assertEqual(body["fitted_on"], "training_only")
        self.assertGreater(body["output_features"], 0)
        fetched = self.client.get(f"/api/preprocessing/runs/{body['id']}")
        self.assertEqual(fetched.status_code, 200)

    def test_target_required_error_is_structured(self) -> None:
        uploaded = self.client.post(
            "/api/datasets/upload",
            files={"file": ("study.csv", b"x,target\n1,0\n2,1\n", "text/csv")},
        )
        response = self.client.post(
            "/api/preprocessing/run",
            json={"dataset_id": uploaded.json()["id"]},
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["error"], "TARGET_REQUIRED")
