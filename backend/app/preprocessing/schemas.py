...
class PreprocessingConfig(BaseModel):
    ...
    feature_selection_method: Literal["none","variance","anova","mutual_info"] = "none"
    feature_count: int = Field(default=8, ge=1, le=512)
    variance_threshold: float = Field(default=0.0, ge=0)
    reduction_method: Literal["none","pca"] = "none"
    pca_components: int = Field(default=4, ge=1, le=128)
    @model_validator(mode="after")
    def validate_outliers(self):
        if self.outlier_mode != "keep" and self.outlier_method is None:
            raise ValueError("outlier_method is required when outlier_mode is clip or remove")
        return self
...
class PreprocessingRun(BaseModel):
    ...
    train_rows: int
    test_rows: int
    train_rows_before_resampling: int
    train_rows_after_resampling: int
    resampling_method: str
    resampling_rows_added: int
    resampling_rows_removed: int
    outliers_removed_from_train: int
    input_features: int
    ...
    train_class_distribution_before_resampling: dict[str,int]
    train_class_distribution_after_resampling: dict[str,int]
