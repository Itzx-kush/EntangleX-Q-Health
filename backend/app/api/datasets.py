...
logger=logging.getLogger(__name__);router=APIRouter(prefix='/api/datasets',tags=['datasets']);_service:DatasetService|None=None
def get_dataset_service()->DatasetService:
 global _service
 if _service is None:_service=DatasetService()
 return _service
async def read_limited(file:UploadFile)->bytes:
 max_bytes=settings.max_upload_size_mb*1024*1024;chunks:list[bytes]=[];total=0
 while chunk:=await file.read(1024*1024):
  total+=len(chunk)
  if total>max_bytes:raise DatasetError('FILE_TOO_LARGE','The uploaded file exceeds the configured size limit.',f'Maximum size is {settings.max_upload_size_mb} MB.',413)
  chunks.append(chunk)
...
async def upload_dataset(file:UploadFile=File(...),target_column:str|None=Form(default=None),service:DatasetService=Depends(get_dataset_service)):
 started=perf_counter();safe_name=(file.filename or 'unnamed')[:255];logger.info('dataset_upload_received filename=%r content_type=%r',safe_name,file.content_type)
 try:
  read_started=perf_counter();content=await read_limited(file);logger.info('dataset_upload_read filename=%r size_bytes=%d read_seconds=%.6f',safe_name,len(content),perf_counter()-read_started)
  result=await run_in_threadpool(service.ingest,file.filename,content,file.content_type,target_column)
  logger.info('dataset_upload_completed dataset_id=%s filename=%r total_seconds=%.6f',result['id'],safe_name,perf_counter()-started);return result
 except DatasetError as exc:logger.warning('dataset_upload_failed filename=%r code=%s total_seconds=%.6f',safe_name,exc.code,perf_counter()-started);raise
 except Exception as exc:logger.exception('dataset_upload_failed filename=%r exception_type=%s total_seconds=%.6f',safe_name,type(exc).__name__,perf_counter()-started);raise
