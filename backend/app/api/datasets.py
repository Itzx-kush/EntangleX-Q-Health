...
async def upload_dataset(
    ...
    """Register a dataset without blocking FastAPI's event loop on pandas/SQLite work."""
    content = await read_limited(file)
    filename = file.filename
    content_type = file.content_type
    return await run_in_threadpool(service.ingest, filename, content, content_type, target_column)
...
def select_target(
    dataset_id: str,
    payload: TargetSelectionRequest,
    service: DatasetService = Depends(get_dataset_service),
):
