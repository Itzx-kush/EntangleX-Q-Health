...
VQC_METRIC_FIELDS = (
    ...
    "sensitivity", "specificity", "roc_auc", "true_negative",
...
def qsvm_service():
    global _qsvm
    if _qsvm is None: _qsvm = QSVMService()
    return _qsvm

def qnn_service():
    global _qnn
    if _qnn is None: _qnn = QNNService()
    return _qnn

def serialize_quantum_registry_record(record):
    model_type = record.get("model_type")
    if model_type == "vqc":
        item = VQCService._response(record)
        item["model_type"] = "vqc"
        item["metrics"] = {field: item.get(field) for field in VQC_METRIC_FIELDS}
        return item
    if model_type == "qsvm":
        return QSVMService._response(record)
    if model_type == "qnn":
        return QNNService._response(record)
    raise QuantumRuntimeError(
        "UNSUPPORTED_QUANTUM_MODEL_TYPE",
        "A persisted quantum model has an unsupported model_type and cannot be listed safely.",
        {"model_type": model_type, "run_id": record.get("id")},
        500,
    )

@router.post("/encodings", response_model=EncodingRun, status_code=201)
def encode(payload: EncodingRequest, service=Depends(encoding_service)): return service.create(payload)

@router.get("/encodings/{run_id}", response_model=EncodingRun)
def get_encoding(run_id: str, service=Depends(encoding_service)): return service.get(run_id)

@router.post("/vqc/train", response_model=VQCRun, status_code=201)
def train_vqc(payload: VQCRequest, service=Depends(vqc_service)): return service.train(payload)
...
def get_vqc(run_id: str, service=Depends(vqc_service)): return service.get(run_id)

@router.post("/qsvm/train", response_model=QuantumModelRun, status_code=201)
def train_qsvm(payload: QSVMRequest, service=Depends(qsvm_service)): return service.train(payload)

@router.get("/qsvm/{run_id}", response_model=QuantumModelRun)
def get_qsvm(run_id: str, service=Depends(qsvm_service)): return service.get(run_id)

@router.post("/qnn/train", response_model=QuantumModelRun, status_code=201)
def train_qnn(payload: QNNRequest, service=Depends(qnn_service)): return service.train(payload)

@router.get("/qnn/{run_id}", response_model=QuantumModelRun)
def get_qnn(run_id: str, service=Depends(qnn_service)): return service.get(run_id)
...
def list_quantum_models(encoding_run_id: str | None = None, service=Depends(qsvm_service)):
    records = service.runs.list_registry_models(encoding_run_id)
    items = [serialize_quantum_registry_record(record) for record in records]
    return {"items": items, "total": len(items)}
