...
class QuantumRuntimeService:
 ...
 def status(self):
  q=find_spec('qiskit') is not None;a=find_spec('qiskit_aer') is not None;i=find_spec('qiskit_ibm_runtime') is not None;t=bool(os.getenv('QISKIT_IBM_TOKEN'))
  return{'simulator_available':q and a,'qiskit_available':q,'ibm_runtime_available':i,'ibm_token_configured':t,'real_quantum_enabled':settings.enable_real_quantum,'provider_ready':i and t and settings.enable_real_quantum,'default_backend':settings.quantum_backend,'default_qubits':settings.default_qubits,'max_qubits':settings.max_qubits,'max_shots':settings.max_quantum_shots}
 def run_diagnostic(self,c):
  ...
  if c.backend!='aer_simulator':
   if not self.status()['provider_ready']:raise QuantumRuntimeError('IBM_RUNTIME_NOT_READY','IBM Quantum execution is disabled or not configured. Use the simulator or configure the optional provider.')
   raise QuantumRuntimeError('IBM_EXECUTION_DEFERRED','Hardware submission is deferred until explicit backend selection is available.')
  if not self.status()['simulator_available']:raise QuantumRuntimeError('QUANTUM_SIMULATOR_UNAVAILABLE','Install the Phase 8 Qiskit and Aer dependencies.')
  from qiskit import QuantumCircuit,transpile
  from qiskit_aer import AerSimulator
  circuit=QuantumCircuit(c.qubits,c.qubits,name='ghz_diagnostic');circuit.h(0)
  for n in range(1,c.qubits):circuit.cx(n-1,n)
  circuit.measure(range(c.qubits),range(c.qubits));backend=AerSimulator(seed_simulator=c.seed);compiled=transpile(circuit,backend,optimization_level=1,seed_transpiler=c.seed);started=perf_counter();result=backend.run(compiled,shots=c.shots,seed_simulator=c.seed).result();duration=round(perf_counter()-started,6);counts={str(k):int(v) for k,v in sorted(result.get_counts().items())};resources={'circuit_name':circuit.name,'circuit_depth':int(compiled.depth()),'circuit_size':int(compiled.size()),'operation_counts':{str(k):int(v) for k,v in compiled.count_ops().items()}};i=str(uuid.uuid4());artifact=self.artifact_dir/f'{i}.json';payload={'backend':'aer_simulator','execution_mode':'simulator','configuration':c.model_dump(),'resources':resources,'measurement_counts':counts,'execution_duration_seconds':duration};artifact.write_text(json.dumps({'run_id':i,**payload},indent=2));record={'id':i,**payload,'artifact_path':f'quantum/{i}.json','created_at':datetime.now(timezone.utc).isoformat()};self.runs.create(record);return self._response(record)
 def get(self,i):
  r=self.runs.get(i)
