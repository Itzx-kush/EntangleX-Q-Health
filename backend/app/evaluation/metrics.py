...
def evaluate_binary(y_true,y_pred,scores=None):
 y_true=np.asarray(y_true);y_pred=np.asarray(y_pred);tn,fp,fn,tp=[int(x) for x in confusion_matrix(y_true,y_pred,labels=[0,1]).ravel()];specificity=tn/(tn+fp) if tn+fp else 0.;auc=float(roc_auc_score(y_true,np.asarray(scores))) if scores is not None and len(np.unique(y_true))==2 else None
