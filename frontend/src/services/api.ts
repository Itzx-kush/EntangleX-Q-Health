...
export async function runPreprocessing(configuration:PreprocessingConfig):Promise<PreprocessingRun>{return parse(await fetch(`${API_URL}/api/preprocessing/run`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(configuration)}))}
